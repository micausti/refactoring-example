...
case class Cart(customer: Customer) {
  ...
  def regularSubtotal(code: PriceCode): Double = {
    customer.rentals
      .map(_.daysRented)
      .map { d =>
        if (d > REGULAR_DISCOUNT_THRESHOLD)
          (d - REGULAR_DISCOUNT_THRESHOLD) * REGULAR_DISCOUNT_MULTIPLIER + REGULAR_PRICE
        else REGULAR_PRICE
      }
      .sum
  }
  ...
  def childrensSubtotal(code: PriceCode): Double = {
    customer.rentals
      .map(_.daysRented)
      .map { d =>
        if (d > CHILDRENS_DISCOUNT_THRESHOLD)
          (d - CHILDRENS_DISCOUNT_THRESHOLD) * CHILDRENS_DISCOUNT_MULTIPLIER + CHILDRENS_PRICE
